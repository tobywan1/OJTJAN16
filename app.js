...
const DropdownMenuWithInputsAndVehiclesAndSubmit = () => {
  ...
  const handleAddNewData = async () => {
    ...
    try {
      const sendData = isAdmin ? { ...newData, isAdmin } : newData;
    ...
    if (lowerCaseUsername === 'admin' && loginPassword === 'wa') {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setIsLoggedIn(true);
          setIsAdmin(data.isAdmin); // Adjust this based on your server response
          setIsCreateAccount(false);
...
if (
  ...
  return (
    ...
    {isLoggedIn ? (
       ...
       <div style={isAdmin ? adminPageStyle :{}} >
         {isAdmin && (
            <>
         <h2>Add New Data</h2>
         ...
         <input
           type="text"
           value={newData.entry}
           onChange={(e) => setNewData({ ...newData, entry: e.target.value })}
         />
         <label>Exit:</label>
         <input
           type="text"
           value={newData.exit}
           onChange={(e) => setNewData({ ...newData, exit: e.target.value })}
         />
         <label>Vehicle:</label>
         <input
           type="text"
